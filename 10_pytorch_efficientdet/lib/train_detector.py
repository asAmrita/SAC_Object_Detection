...
class Detector():
    ...
    def set_fixed_params(self):
        ...
        self.system_dict["params"]["anchors_ratios"] = '[(0.9, 1.1), (1.0, 1.0), (1.2, 0.9)]';
...
# Your model must be on the same device
        model.eval()
        model = model.to(device)
       ...
       # print(summary(model, input_size=(3, 512, 512)))
        ...
        print("");
        print("");
        print(summary(SummaryWrapper(model), input_size=(3, 512, 512)))
        print(summary(model, input_size=(3, 512, 512)))
        ...
        #scheduler= torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=100, eta_min=0, last_epoch=- 1, verbose=True)
